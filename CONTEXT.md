...
- **Stale `performedAt` after a Workout date edit** — Editing a Workout's `date` does not currently propagate to its child Sets' `performedAt`. Per [ADR-0001](./docs/adr/0001-denormalize-performed-at-onto-set.md) we keep the denormalization, so the propagation needs to be added in `WorkoutsDataSlice.update`. Filed as a follow-up.
...
- **`isAdHoc` flag is redundant** — `EditorExerciseGroup.templateMeta.isAdHoc` only exists to mark "this group has no **Set Target**" so the editor suppresses the "Target: L–U reps" line. There is no user-visible concept of an ad-hoc exercise. Filed as a cleanup: replace the boolean with the presence/absence of the Set Target reference itself (e.g. rename `templateMeta` → `setTarget` and treat `null` as the ad-hoc case).
